...
void Tower::Fire(){
    ...
    double A = -dy;
    double B = dx;
    double C = -(A * target->x + B * target->y);
    for(std::list<Ant>::iterator i=ants.begin();i!=ants.end();++i){
        if(Cross(A, B, C, i->x, i->y)&&InSegment(this->x,this->y,target->x,target->y,i->x,i->y)&&SqrEucDis(this->x,this->y,i->x,i->y)<=SqrEucDis(this->x,this->y,target->x,target->y)){
            i->HP-=d;
        }
    }
}
